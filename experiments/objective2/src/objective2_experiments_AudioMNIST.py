# objective2_experiments_audioMNIST.py
...
class XAIExperiment:
    def __init__(self, model_path, test_data_path, test_labels_path, results_dir, baseline_mode="zero"):
    ...
    def generate_explanations(self, method, inputs, targets):
        ...
        if method == "IntegratedGradients":
            baseline = self.get_custom_baseline(inputs)
            return self.xai_methods[method].attribute(inputs, baselines=baseline, target=targets).cpu().detach().numpy()
...
def main():
    ...
    experiment = XAIExperiment(
        ...
        baseline_mode="median"  # options: "zero", "median", "low_energy_mask", "noisy_silence"
    )
